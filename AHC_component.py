...
def graph_init(n, p, percent, balance_weight_grp1, balance_weight_grp2):
	...
	lst = list(nx.connected_components(graph)) # lista składowych spójności
	max_component = max(lst, key=len) # największa składowa
	
	H = graph.subgraph(max_component)
	
	#print('graf wezly:', graph.nodes())
	#print('graf krawedzie :', graph.edges())
	#print('podgraf wezly:', H.nodes())
	#print('podgraf krawedzie:', H.edges())
	#print('lista skladowych spojnosci :', lst)
	#print('najwieksza skladowa :', max_component)
